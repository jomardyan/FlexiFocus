...
function init() {
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg.type === 'stateUpdated') {
      appState = { state: msg.state, settings: msg.settings, methods: msg.methods };
      render();
    }
  });

  els.method.addEventListener('change', async () => {
    await chrome.runtime.sendMessage({ type: 'setMethod', methodKey: els.method.value });
  });

  els.primary.addEventListener('click', handlePrimary);
  els.secondary.addEventListener('click', () => chrome.runtime.sendMessage({ type: 'resetTimer' }));
  els.flowComplete.addEventListener('click', () => chrome.runtime.sendMessage({ type: 'completeFlowtime' }));
...
function renderTimer(state, methods, initialRemaining) {
  ...
  els.time.textContent = formatTime(timer.methodKey === 'flowtime' ? remainingMs : remainingMs);
  ...
  els.primary.textContent = isRunning ? 'Pause' : shouldResume ? 'Resume' : 'Start';
  els.secondary.textContent = 'Reset';
  ...
  ticker = setInterval(() => {
    ...
    els.time.textContent = formatTime(state.timer.methodKey === 'flowtime' ? remaining : remaining);
  }, 1000);
}
...
function handlePrimary() {
  ...
  chrome.runtime.sendMessage({ type: 'startTimer', methodKey, phase: state.timer.phase });
}

function computeDuration(timer, method) {
  if (method.flexible) return Math.max(1, timer.isRunning ? Date.now() - timer.startTime : timer.remainingMs || 0);
...
function computeRemaining(timer, method, overrideRemaining) {
  ...
  return Math.max(0, timer.endTime ? timer.endTime - Date.now() : computeDuration(timer, method));
}
...
function buildStatus(timer, method) {
  if (method.flexible) {
    return timer.isRunning ? 'Flowtime running — end when you feel ready.' : 'Start flow and end to calculate break.';
  }
...
function renderTasks(tasks = [], activeTaskId) {
  ...
  tasks.forEach((task) => {
    ...
    checkbox.addEventListener('change', () => {
      chrome.runtime.sendMessage({ type: 'updateTask', id: task.id, updates: { done: !task.done } });
    });
    ...
    focusBtn.addEventListener('click', () => chrome.runtime.sendMessage({ type: 'setActiveTask', id: task.id }));

    const delBtn = document.createElement('button');
    delBtn.className = 'btn tiny ghost';
    delBtn.textContent = 'Delete';
    delBtn.addEventListener('click', () => chrome.runtime.sendMessage({ type: 'deleteTask', id: task.id }));
...
function renderHistory(history = [], methods = {}) {
  ...
  history.slice(0, 6).forEach((entry) => {
    ...
    label.innerHTML = `<strong>${(methods[entry.methodKey]?.label) || entry.methodKey}</strong> • ${capitalize(entry.phase)}`;
    const meta = document.createElement('div');
    meta.className = 'meta';
